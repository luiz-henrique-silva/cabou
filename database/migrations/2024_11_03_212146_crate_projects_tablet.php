...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('projects', function (Blueprint $table) {
            $table->id();
            $table->string('image')->nullable(); // Caminho para a imagem
            ...
            $table->unsignedBigInteger('materia_id')->nullable(); // ID da matéria
            $table->foreign('user_id')->references('id')->on('users')->onDelete('cascade');
            $table->foreign('materia_id')->references('id')->on('materias')->onDelete('set null'); // Chave estrangeira para a tabela 'materias'
            $table->timestamps();
        });
    }
